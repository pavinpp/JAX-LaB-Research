...
def parse_ui_args():
    ...
    parser.add_argument("--dx_um", type=float, default=10.0, help="Voxel size in micrometers (um)")
...
# -------------------------------------------------------------------
# ฟังก์ชันคำนวณความหนืดตามอุณหภูมิ (JIT-compatible)
# -------------------------------------------------------------------
@jit
def calculate_tau_f(T_celsius, tau_ref=1.0):
    """
    คำนวณค่า tau_f สำหรับ LBM โดยอ้างอิงจากตารางความหนืดของ CuSO4
    """
    temp_points = jnp.array([25.0, 35.0, 45.0, 55.0, 65.0, 75.0])
    viscosity_points = jnp.array([1.35, 1.08, 0.89, 0.74, 0.63, 0.55])
    
    # ใช้ jnp.interp เพื่อให้สามารถ Auto-diff และรันบน GPU ได้เต็มที่
    mu_T = jnp.interp(T_celsius, temp_points, viscosity_points)
    
    mu_ref = 1.35 # ความหนืดอ้างอิง ณ 25 องศาเซลเซียส
...
def run_simulation():
    ...
    dt_s = (nu_lb * (dx_m ** 2)) / nu_phys  # เวลาเป็นวินาที
    ...
    def lbm_step(state, step_idx):
        ...
        # --- Soft Start (Velocity Ramping) ---
        # ค่อยๆ เพิ่มความเร็วในช่วง 500 step แรก (0.0 -> 1.0)
        ...
        # คำนวณ tau_f เป็น JAX Array ที่ผันแปรตามอุณหภูมิในแต่ละจุด (Voxel)
        tau_f_local = calculate_tau_f(T, tau_ref=tau_f_ref)
        omega_f_local = 1.0 / tau_f_local
        
        # ปรับการชน (Collision) ให้ใช้ omega_f_local ที่เปลี่ยนตามพื้นที่
        ...
        if args.axis == 'X':
            # ใช้ current_u_lb ที่ผ่านการ Ramp แล้วสำหรับทางเข้า
    ...
    # ส่งตัวแปร global_step (chunk_start) เพื่อให้ scan รู้ว่าอยู่ step ที่เท่าไหร่สำหรับ Ramping
    ...
    for i in range(num_chunks + 1):
        ...
        if current_step == 0 or current_step % 500 == 0:
            ...
            # คำนวณความหนืดเฉลี่ยสำหรับการประเมิน Permeability
            avg_T = np.mean(T_np)
            ...
            time_s = current_step * dt_s # ใช้หน่วยเป็นวินาที
            ...
            # แสดงผล Time เป็นวินาที (s)
            print(f"Time: {time_s:.2f} s | Porosity: {porosity:.4f} | Crystals: {num_features} | k: {k_perm_darcy:.2e} Darcy | Max Supersat: {max_supersat:.4f} | Crystal Vol: {current_solid_vol_mm3:.2e} mm3")
...
def generate_reaction_maps(maps_data, mask_np):
    print("\nGenerating 3x3 Spatial Reaction Maps for XY, XZ, and YZ planes...")
...
def generate_analytical_plots(vel_t0, vel_tfinal, pe_da_data, u_scale_mms):
    ...
    # 1. Permeability Reduction (Darcy vs Seconds)
    if kinetics.ndim > 1:
        time_s = kinetics[:, 1]
    ...
    # 2. Velocity Distribution Shift (mm/s)
    plt.figure(figsize=(8, 6))
    ...
    # 3. Morphology Trajectory (Volume mm3 vs SA/V)
