...
def run_simulation():
    ...
    print(f"Loading full geometry from {args.geom}...")
    mask_np_full = np.load(args.geom).astype(bool)
    
    # We NO LONGER crop. Use the full geometry natively.
    mask = jnp.array(mask_np_full)
    nx, ny, nz = mask.shape
    print(f"Full Domain Size: {nx}x{ny}x{nz}")
    
    # Calculate the exact bounds for the centered injection window on the full domain
    half_w = args.inject_size // 2
    cx, cy, cz = nx // 2, ny // 2, nz // 2
    
    # Static Python slices for JAX compilation
    xs, xe = int(max(0, cx - half_w)), int(min(nx, cx + half_w))
    ys, ye = int(max(0, cy - half_w)), int(min(ny, cy + half_w))
    zs, ze = int(max(0, cz - half_w)), int(min(nz, cz + half_w))
    
    print(f"Injection Window [{args.inject_size}x{args.inject_size}] placed at Y[{ys}:{ye}], Z[{zs}:{ze}]")
    
    lattice = LatticeD3Q19()
    c_int = np.array(lattice.c, dtype=int).T.tolist()   # Pure Python list for static slicing
    c = jnp.array(lattice.c, dtype=jnp.float32).T       # Float version for equilibrium math
    ...
    def lbm_step(state, _):
        ...
        # 2. Impose UI Inlet manually for stability (ONLY inside the injection window)
        if args.axis == 'X':
            u = u.at[0, ys:ye, zs:ze, :].set(target_u)
            rho = rho.at[0, ys:ye, zs:ze].set(1.0)
            T = T.at[0, ys:ye, zs:ze].set(T_hot)
            C = C.at[0, ys:ye, zs:ze].set(C_inlet)
        ...
        # Mark side planes as solid walls so they participate in bounce-back
        wall_mask = jnp.zeros_like(effective_fluid_mask, dtype=bool)
        if args.axis == 'X':
            # 4 side walls of the channel
            wall_mask = wall_mask.at[:, 0, :].set(True)
            wall_mask = wall_mask.at[:, -1, :].set(True)
            wall_mask = wall_mask.at[:, :, 0].set(True)
            wall_mask = wall_mask.at[:, :, -1].set(True)
            
            # The Front and Back faces MUST be walls EXCEPT for the injection window
            face_wall = jnp.ones((ny, nz), dtype=bool)
            face_wall = face_wall.at[ys:ye, zs:ze].set(False)
            wall_mask = wall_mask.at[0, :, :].set(wall_mask[0, :, :] | face_wall)
            wall_mask = wall_mask.at[-1, :, :].set(wall_mask[-1, :, :] | face_wall)
            ...
            wall_mask = wall_mask.at[:, :, 0].set(True)
            wall_mask = wall_mask.at[:, :, -1].set(True)
            face_wall = jnp.ones((nx, nz), dtype=bool)
            face_wall = face_wall.at[xs:xe, zs:ze].set(False)
            wall_mask = wall_mask.at[:, 0, :].set(wall_mask[:, 0, :] | face_wall)
            wall_mask = wall_mask.at[:, -1, :].set(wall_mask[:, -1, :] | face_wall)
            ...
            wall_mask = wall_mask.at[:, 0, :].set(True)
            wall_mask = wall_mask.at[:, -1, :].set(True)
            face_wall = jnp.ones((nx, ny), dtype=bool)
            face_wall = face_wall.at[xs:xe, ys:ye].set(False)
            wall_mask = wall_mask.at[:, :, 0].set(wall_mask[:, :, 0] | face_wall)
            wall_mask = wall_mask.at[:, :, -1].set(wall_mask[:, :, -1] | face_wall)
            
        effective_fluid_mask_bc = effective_fluid_mask & (~wall_mask)

        # Call heterogeneous precipitation
        ...
        # 6. Inlet and Outlet Boundary Conditions (STRICTLY IN THE WINDOW)
        if args.axis == 'X':
            u_in = jnp.zeros_like(u[0, ys:ye, zs:ze]).at[..., 0].set(u_lb)
            f_str = f_str.at[0, ys:ye, zs:ze, :].set(calc_equilibrium(jnp.ones_like(rho[0, ys:ye, zs:ze]), u_in))
            g_str = g_str.at[0, ys:ye, zs:ze, :].set(calc_equilibrium(jnp.ones_like(T[0, ys:ye, zs:ze]) * T_hot, u_in))
            h_str = h_str.at[0, ys:ye, zs:ze, :].set(calc_equilibrium(jnp.ones_like(C[0, ys:ye, zs:ze]) * C_inlet, u_in))
            
            f_str = f_str.at[-1, ys:ye, zs:ze, :].set(f_str[-2, ys:ye, zs:ze, :])
            g_str = g_str.at[-1, ys:ye, zs:ze, :].set(g_str[-2, ys:ye, zs:ze, :])
            h_str = h_str.at[-1, ys:ye, zs:ze, :].set(h_str[-2, ys:ye, zs:ze, :])
            
        elif args.axis == 'Y':
            u_in = jnp.zeros_like(u[xs:xe, 0, zs:ze]).at[..., 1].set(u_lb)
            f_str = f_str.at[xs:xe, 0, zs:ze, :].set(calc_equilibrium(jnp.ones_like(rho[xs:xe, 0, zs:ze]), u_in))
            g_str = g_str.at[xs:xe, 0, zs:ze, :].set(calc_equilibrium(jnp.ones_like(T[xs:xe, 0, zs:ze]) * T_hot, u_in))
            h_str = h_str.at[xs:xe, 0, zs:ze, :].set(calc_equilibrium(jnp.ones_like(C[xs:xe, 0, zs:ze]) * C_inlet, u_in))
            
            f_str = f_str.at[xs:xe, -1, zs:ze, :].set(f_str[xs:xe, -2, zs:ze, :])
            g_str = g_str.at[xs:xe, -1, zs:ze, :].set(g_str[xs:xe, -2, zs:ze, :])
            h_str = h_str.at[xs:xe, -1, zs:ze, :].set(h_str[xs:xe, -2, zs:ze, :])
            
        elif args.axis == 'Z':
            u_in = jnp.zeros_like(u[xs:xe, ys:ye, 0]).at[..., 2].set(u_lb)
            f_str = f_str.at[xs:xe, ys:ye, 0, :].set(calc_equilibrium(jnp.ones_like(rho[xs:xe, ys:ye, 0]), u_in))
            g_str = g_str.at[xs:xe, ys:ye, 0, :].set(calc_equilibrium(jnp.ones_like(T[xs:xe, ys:ye, 0]) * T_hot, u_in))
            h_str = h_str.at[xs:xe, ys:ye, 0, :].set(calc_equilibrium(jnp.ones_like(C[xs:xe, ys:ye, 0]) * C_inlet, u_in))
            
            f_str = f_str.at[xs:xe, ys:ye, -1, :].set(f_str[xs:xe, ys:ye, -2, :])
            g_str = g_str.at[xs:xe, ys:ye, -1, :].set(g_str[xs:xe, ys:ye, -2, :])
            h_str = h_str.at[xs:xe, ys:ye, -1, :].set(h_str[xs:xe, ys:ye, -2, :])
    ...
    print(f"Running Reactive {args.steps} LBM steps on full domain...")
...
if __name__ == "__main__":
    ...
    fig1.suptitle("Full Domain: Cooling-Induced CuSO4 Crystallization", fontsize=16)
    fig1.tight_layout()
    ...
    fig2.suptitle("Full Domain: LBM Reactive Transport Temporal Evolution", fontsize=16)
    fig2.tight_layout()
    ...
    #Perfect code!!!! Run smoothly, now upscale, we don’t need to crop I want to run full size but inlet and outlet still same size